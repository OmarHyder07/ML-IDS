...
\end{itemize}
These metrics will be used with future tables.
\begin{table}[H]
\centering
\makebox[\textwidth][c]{%
...
\end{table}
\subsection{Verdict}  
While imbalance handling techniques created marked improvements, reaching near perfect attack recall without raising the false-positive rate too high, the aforementioned results show nothing of the model's capacity to classify unseen attacks. Thus, a relevant direction of investigation would be to evaluate the same model on a split in which the test set contains unseen attack types.
...
\end{table}
\paragraph{Results} Table 4: steep drop in anomaly recall. From macro-average attack recalls of 0.81-0.86 on the random split to a macro attack recall of 0.23; with weighted attack recall, a drop from 0.99$\to$0.01. NSL-KDD had a drop of 0.71$\to$0.38 when introduced to unseen types. Here, under a harsher temporal split, it completely collapsed, reaffirming zero-days as the fundamental ceiling on supervised algorithms. The modicum of success on web attacks may be an artifact of resemblance to Wednesday's DoS attacks. 
...
\bottomrule
\end{tabular}}
\caption{Per-class recall; iForest vs. LOF; XGBoost values copied from table 4}
\end{table}
\paragraph{Results analysis} Across the board jumps in anomaly recall for both iForest and LOF from XGBoost, with sharp false-positive rate increases. A real SOC team would drown at 5\% FPR with this volume of traffic. My predictions (on test) with per-class performance based on their inductive biases were correct, except for DDoS in which iForest beat LOF.
\par Clearly, a key drawback of anomaly detection methods for unseen classification is the spike in the false-positive rate. A fairer comparison would pit the two against each other at equal FPRs. In the attempt of fairness, I used both models "score" functions, where a higher number represents a higher likelihood of being anomalous. Using percentile rank transform to scale each score in the range [0, 1], I can sweep through thresholds and plot weighted attack recall vs. FPR. This can be done on cross-validation, and since no tuning is conducted via the graph, also on test.
\begin{figure}[H]
    \centering
...
\par However, on the test set, LOF seems very much more proficient over the PortScan class, and beats iForest on Bot.  This is reflected by figure 3. PortScan flows occupy 55\% of the attacks in test. Thus, LOF's ability to detect these allows it to be stronger on the test set than iForest, with much greater attack recall for the same FPR (of course, at FPRs much beyond reasonable). 
...
\textbf{iForest} & Temporal & 0 of 7 & 0.3744& 0.29& 0.67& 0.46 \\ 
\textbf{LOF} & Temporal & 0 of 7 & 0.4602& 0.40& 0.76& 0.91 \\ 
\bottomrule
\end{tabular}}
...
\par I would test on multiple other datasets to see if my findings generalise further. I would seek other datasets not for differing feature sets, but for the variety of attack classes represented. I could also use the leave-one-attack-family-out method. Here, this would entail the creation of 14 unique test sets where only one attack class is held-out, thus allowing me to further test how specific attack types respond to different model architectures when evaluated unseen. 
...
Raw packet captures were converted using the \textbf{CICFlowMeter} tool into labelled CSVs of $\sim80$ features. See table 8.
